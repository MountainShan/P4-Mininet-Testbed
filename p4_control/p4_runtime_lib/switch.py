...
class SwitchConnection(object):
    ...
    def PacketOut (self, payload=None, dry_run=False):
        request = p4runtime_pb2.StreamMessageRequest()
        pktout = p4runtime_pb2.PacketOut()
        if (payload):
            pktout.payload = payload
        request.packet.CopyFrom(pktout)
        if dry_run:
            print "P4 Runtime PacketOut: ", request 
        else:
            self.requests_stream.put(request)
    ...
    def WriteCloneSessionEntry(self, clone_entry, dry_run=False):
        request = p4runtime_pb2.WriteRequest()
        request.device_id = self.device_id
        request.election_id.low = 1
        update = request.updates.add()
